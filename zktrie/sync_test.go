...
func testIterativeSync(t *testing.T, count int, bypath bool) {
	...
	for len(hashQueue)+len(pathQueue) > 0 {
		...
		for i, path := range pathQueue {
			...
			nodeHash, _ := NodeHash(data)
			results[len(hashQueue)+i] = SyncResult{nodeHash, data}
		}
...
func TestIncompleteSync(t *testing.T) {
	...
	for len(queue) > 0 {
		...
		batch.Write()
		for _, result := range results {
			dbKey, _ := NodeStoreHash(result.Data)
			added = append(added, dbKey[:])
	...
	for _, key := range added[1:] {
		value, _ := diskdb.Get(key)

		diskdb.Delete(key)
		if err := checkTrieConsistency(triedb, srcTrie.Hash()); err == nil {
...
func TestSyncOrdering(t *testing.T) {
	...
	for i := 0; i < len(reqs)-1; i++ {
		...
		if bytes.Compare(compactToBinary(reqs[i][0]), compactToBinary(reqs[i+1][0])) > 0 {
			t.Errorf("Invalid request order: %v before %v", compactToBinary(reqs[i][0]), compactToBinary(reqs[i+1][0]))
		}
	}
}
