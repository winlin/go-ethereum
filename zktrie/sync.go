...
func newSyncPath(path []byte) SyncPath {
	...
	if len(path) < 256 {
		return SyncPath{binaryToCompact(path)}
	}
	return SyncPath{binaryToKeybytes(path[:256]), binaryToCompact(path[256:])}
}
...
func (s *Sync) Process(result SyncResult) error {
	...
	if req := s.nodeReqs[result.Hash]; req != nil && req.data == nil {
		...
		requests, err := s.processNode(req, result.Data)
		if err != nil {
			return err
		}
...
func (s *Sync) schedule(req *request) {
	...
	// prio = | path length | path |
	//           16bit   +   47bit = 63bit
	prio := int64(len(req.path)) << 47 // depth <= 512 = 2^16
	for i := 0; i < 47 && i < len(req.path); i++ {
		prio |= int64(1-req.path[i]) << (47 - i) // lexicographic order
	}
...
func (s *Sync) processNode(req *request, node []byte) ([]*request, error) {
	...
	n, err := itrie.NewNodeFromBytes(node)
	if err != nil {
		return nil, err
	}
	...
	for _, child := range children {
		...
		hash := common.BytesToHash(child.hash.Bytes())
