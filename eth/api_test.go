...
func TestAccountRange(t *testing.T) {
	...
	var (
		statedb  = state.NewDatabaseWithConfig(rawdb.NewMemoryDatabase(), &zktrie.Config{Preimages: true})
	...
	for i := range addrs {
		...
		addr := common.BytesToAddress(crypto.PoseidonSecureHash(hash.Bytes()).Bytes())
		addrs[i] = addr
